...
def get_llm_model(provider: str, **kwargs):
    ...
    if provider == "anthropic":
        ...
        if not kwargs.get("base_url", ""):
            base_url = os.getenv("NVIDIA_ENDPOINT", "https://integrate.api.nvidia.com/v1")
        else:
            base_url = kwargs.get("base_url")

        return ChatOpenAI(
            model=kwargs.get("model_name", "meta/llama-3.3-70b-instruct"),
            temperature=kwargs.get("temperature", 0.0),
            base_url=base_url,
            api_key=api_key or os.getenv("NVIDIA_API_KEY") or os.getenv("NVAPI_KEY"),
        )
    elif provider == "google":
        return ChatGoogleGenerativeAI(
            model=kwargs.get("model_name", "gemini-3.1-flash-lite"),
            temperature=kwargs.get("temperature", 0.0),
            api_key=api_key,
        )
